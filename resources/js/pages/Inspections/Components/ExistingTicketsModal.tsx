...
interface ExistingTicketsModalProps {
  ...
  onPing: (ticket: Ticket) => void;
  onReportNew: () => void;
  openTickets: Ticket[];
}

export function ExistingTicketsModal({ isOpen, onOpenChange, onPing, onReportNew, openTickets }: ExistingTicketsModalProps) {
  const handlePing = (ticket: Ticket) => {
    onPing(ticket);
    onOpenChange(false);
  };
            ...
            <CarouselContent>
              {openTickets.map((ticket) => (
                <CarouselItem key={ticket.id} className="md:basis-1/2 lg:basis-1/3">
                  <div className="p-1">
                    <TicketMiniCard ticket={ticket} onClick={() => handlePing(ticket)} />
                  </div>
                </CarouselItem>
              ))}
            </CarouselContent>

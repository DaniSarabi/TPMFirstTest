...
// --- ACTION: Import the TicketStatus type ---
import { TicketStatus } from './Columns';

// --- ACTION: Update the props to be specific to TicketStatus ---
interface ReassignAndDeleteStatusModalProps {
    isOpen: boolean;
    onOpenChange: (isOpen: boolean) => void;
    onConfirm: (newStatusId: number) => void;
    statusToDelete: TicketStatus | null;
    otherStatuses: TicketStatus[];
}

export function ReassignAndDeleteStatusModal({
    isOpen,
    onOpenChange,
    onConfirm,
    statusToDelete,
    otherStatuses,
}: ReassignAndDeleteStatusModalProps) {
        ...
        <AlertDialog open={isOpen} onOpenChange={onOpenChange}>
            <AlertDialogContent className='bg-popover'>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        This status may be in use. To delete it, please choose a new status to assign to all affected tickets. This action cannot be undone.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <div className="py-4">
                    <Label htmlFor="new-status-select">Re-assign tickets to:</Label>
                    <Select value={newStatusId ? String(newStatusId) : ''} onValueChange={(value) => setNewStatusId(Number(value))}>
                        <SelectTrigger className='ring-1 ring-ring' id="new-status-select">

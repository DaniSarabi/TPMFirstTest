...
export function TicketMiniCard({ ticket }: TicketMiniCardProps) {
  ...
  return (
    <Link href={route('tickets.show', ticket.id)}>
     <Card className="flex h-full w-full flex-col overflow-hidden  border-0 p-0 shadow-md drop-shadow-lg hover:bg-accent transition-transform ease-in-out hover:-translate-y-1 ">
        ...
        <CardContent className="flex flex-col p-4 pt-2">
          ...
          <p className="line-clamp-2 text-xs text-muted-foreground">{ticket.machine.name}</p>

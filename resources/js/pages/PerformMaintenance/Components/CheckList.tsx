    ...
    <form className="space-y-6">
      {isOverdue && (
        <div className="flex items-center gap-4 rounded-md bg-yellow-100 p-4 text-yellow-600">
          ...
          <div>
            <h1 className='font-semibold text-xl'>Maintenance Overdue</h1>
            <p className='text-yellow-800'>This task is past its due date. Please complete it as soon as possible and add a note explaining the reason for the delay.</p>
          </div>
        </div>
      )}
      <Card>
          ...
          {scheduledMaintenance.template.tasks.map((task: MaintenanceTemplateTask, index: number) => {
            ...
            return (
              <TaskItemRenderer
                ...
                errors={errors}
                onPhotoClick={() => {
                  /* Logic to open camera modal would go here */
                }}

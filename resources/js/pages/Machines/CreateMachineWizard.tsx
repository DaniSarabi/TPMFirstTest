    ...
    return (
        <ol className="flex items-center w-full text-sm font-medium text-center text-gray-500 dark:text-gray-400 sm:text-base">
            {steps.map((stepName, index) => {
                ...
                return (
                    <li key={stepNumber} className={`flex md:w-full items-center ${isCompleted ? 'text-blue-600 dark:text-blue-500' : ''} ${index < steps.length - 1 ? "sm:after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-6 xl:after:mx-10 dark:after:border-gray-700" : ''}`}>
                        <span className={`flex items-center after:content-['/'] sm:after:hidden after:mx-2 after:text-gray-200 dark:after:text-gray-500 ${isCurrent ? 'font-bold' : ''}`}>
                            {isCompleted ? (
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="space-y-2">
                            ...
                            <div className="flex space-x-2">
                                <Input id="subsystem-name" value={subsystemName} onChange={(e) => setSubsystemName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddSubsystem(); } }} />
                                <Button type="button" onClick={handleAddSubsystem}>Add</Button>
                            </div>
                            {errors.subsystems && <p className="text-sm text-red-500 mt-1">{errors.subsystems[0]}</p>}
                        </div>
                        <div className="space-y-2">
                            ...
                            {subsystems.length > 0 ? (
                                <div className="space-y-2 rounded-md border p-2">
                                    {subsystems.map((name, index) => (
                                        <div key={index} className="flex items-center justify-between bg-primary text-white p-2 rounded">
                                            <span>{name}</span>
                                ...
                                <p className="text-sm text-muted-foreground text-center py-4">No subsystems added yet.</p>
                            )}
                        </div>
                    </div>
                );
            case 3:
                return (
                    <div className="grid gap-4 py-4">
                        <Tabs defaultValue={createdSubsystems[0]?.id.toString()} className="w-full overflow-x-auto">
                            <TabsList className="x-max">
                                {createdSubsystems.map((sub) => (
                                    <TabsTrigger key={sub.id} value={sub.id.toString()}>{sub.name}</TabsTrigger>
                                ))}
                            </TabsList>
                            {createdSubsystems.map((sub) => (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex space-x-2">
                                                <Input
                                                    ...
                                                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddInspectionPoint(sub.id); } }}
                                                />
                                                <Button type="button" onClick={() => handleAddInspectionPoint(sub.id)}>Add Point</Button>
                                            </div>
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            {(inspectionPoints[sub.id]?.length ?? 0) > 0 ? (
                                                <div className="bg-pri space-y-2 rounded-md border p-2">
                                                    {inspectionPoints[sub.id].map((name, index) => (
                                                        <div key={index} className="flex items-center justify-between bg-primary text-white p-2 rounded">
                                                            <span>{name}</span>
                                                            <Button variant="ghost" size="icon" className="h-6 w-6 hover:bg-red-500" onClick={() => handleRemoveInspectionPoint(sub.id, index)}>
                                                                <X className="h-4 w-4" />
                                                            </Button>
                                                        </div>
                                                    ))}
                                                ...
                                                <p className="text-sm text-muted-foreground text-center py-4">No inspection points added yet for this subsystem.</p>
                                            )}
                                        </div>
                                    </div>
                                </TabsContent>
                        ...
                        {errors.inspection_points && <p className="text-sm text-red-500 mt-2">{errors.inspection_points[0]}</p>}
                    </div>
                );
            default:
                return null;
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Follow the steps to add a new machine and its components to the system.
                    </DialogDescription>
                </DialogHeader>
                <div className="p-4">
                ...
                <DialogFooter>
                    {step > 1 && (
                        <Button variant="outline" onClick={() => setStep(step - 1)} disabled={processing}>Back</Button>
                    )}
                    {step === 1 && (
                    ...
                    {step === 2 && (
                        <>
                            <Button variant="secondary" className='text-white' onClick={handleClose} disabled={processing}>Finish</Button>

                    ...
                    <form id="subsystem-form" onSubmit={handleSaveSubsystem} className="grid gap-4 py-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                autoFocus
                            />
                            {subsystemErrors.name && <p className="text-sm text-red-500 mt-1">{subsystemErrors.name[0]}</p>}
                        </div>
                    </form>
                );
            case 2:
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex space-x-2">
                                <Input
                                    ...
                                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddInspectionPoint(); } }}
                                />
                                <Button type="button" onClick={handleAddInspectionPoint}>Add</Button>
                            </div>
                        </div>
                        <div className="space-y-2">
                            ...
                            {inspectionPoints.length > 0 ? (
                                <div className="space-y-2 rounded-md border p-2 max-h-48 overflow-y-auto">
                                    {inspectionPoints.map((name, index) => (
                                        <div key={index} className="flex items-center justify-between bg-accent text-accent-foreground p-2 rounded">
                                            <span>{name}</span>
                                ...
                                <p className="text-sm text-muted-foreground text-center py-4">No inspection points added yet.</p>
                            )}
                        </div>
                    </div>
                );
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Follow the steps to add a new subsystem and its inspection points.
                    </DialogDescription>
                </DialogHeader>
                <div className="p-4">

...
import { User as GlobalUserType } from '@/types'; // Import the global User type
      ...
      if (priority === 2) {
        return (
          <div className="flex items-center gap-2 text-lg text-red-600">
            <ShieldAlert className="h-6 w-6" /> Critical
          </div>
        );
      }
      if (priority === 1) {
        return (
          <div className="flex items-center gap-2 text-lg text-yellow-600">
            <AlertTriangle className="h-6 w-6" /> Needed
          </div>
        );
      }
